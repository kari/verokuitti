...
for (i in seq_len(nrow(depts))) {
  ...
  budget <- tibble(
    l1 = dept,
    l2 = tbl |>
      map(\(x) x[1] |> html_element("span") |> html_text()) |>
      unlist(),
    l3 = tbl |> map(\(x) x[1] |> html_text()) |> unlist(),
...
write_csv(output, output_file, col_names = FALSE)

# määrärahat yhteensä
output |> filter(is.na(l2) & is.na(l3)) |> summarise(sum = sum(sum))
